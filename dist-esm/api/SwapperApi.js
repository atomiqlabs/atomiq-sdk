...
export class SwapperApi {
    constructor(swapper, config) {
        ...
        this.endpoints = {
            ...
            listSwaps: createApiEndpoint("GET", "List all swaps for a given signer address. Returns an array of swap objects, each with swapId, swapType, state, quote, steps, and terminal state flags (isFinished, isSuccess, isFailed, isExpired). Optionally filter by smart chain.", this.listSwaps.bind(this), {
            ...
            listPendingSwaps: createApiEndpoint("GET", "List swaps that require user action for a given signer address. Returns an array of swap objects with the same structure as listSwaps.", this.listPendingSwaps.bind(this), {
            ...
            getSupportedTokens: createApiEndpoint("GET", "List all tokens available as swap input or output. Returns an array of ApiToken objects, each with id (e.g. BITCOIN-BTC, LIGHTNING-BTC, STARKNET-STRK), chainId, ticker, name, decimals, and address.", this.getSupportedTokens.bind(this), {
                side: {
                    type: "string",
                    required: true,
            ...
            getSwapCounterTokens: createApiEndpoint("GET", "Get tokens that can be swapped against a given token. Returns an array of ApiToken objects (id, chainId, ticker, name, decimals, address). Use to discover valid trading pairs.", this.getSwapCounterTokens.bind(this), {
                token: {
                    type: "string",
                    required: true,
            ...
            getSwapLimits: createApiEndpoint("GET", "Get minimum and maximum swap amounts for a source/destination token pair. Returns {input: {min, max?}, output: {min, max?}} where each value is an ApiAmount object with amount (decimal string), rawAmount (base units string), decimals, symbol, and chain.", this.getSwapLimits.bind(this), {
            ...
            parseAddress: createApiEndpoint("GET", "Parse and validate an address, Lightning invoice, LNURL, or Bitcoin URI. Returns {address, type} and optionally lnurl (ApiLNURL with pay/withdraw details), min/max/amount (as ApiAmount objects).", this.parseAddress.bind(this), {
                address: { type: "string", required: true, description: "Address, invoice, LNURL, or URI string to parse" }
            }),
            getSpendableBalance: createApiEndpoint("GET", "Get the spendable balance for a wallet address and token, accounting for chain fees. Returns {balance: ApiAmount, feeRate?} where ApiAmount has amount (decimal string), rawAmount (base units string), decimals, symbol, and chain.", this.getSpendableBalance.bind(this), {
            ...
            submitTransaction: createApiEndpoint("POST", "Submit signed transaction(s) for a swap. Call this after the user has signed the transaction returned by getSwapStatus. Returns {txHashes: string[]} with the submitted transaction hashes. After submission, continue polling getSwapStatus.", this.submitTransaction.bind(this), {
                swapId: { type: "string", required: true, description: "The swap identifier" },
                signedTxs: {
                    type: "array",
                    required: true,
            ...
            settleWithLnurl: createApiEndpoint("POST", "Settle a Lightning Network swap using an LNURL-withdraw link. Returns {paymentHash: string} on success.", this.settleWithLnurl.bind(this), {
                swapId: { type: "string", required: true, description: "The swap identifier" },
