import { isOptionalField, verifyField } from "../SchemaVerifier.js";
 ...
 */
export async function streamingFetchPromise(url, body, schema, timeout, signal, streamRequest, _headers = {}) {
    if (streamRequest == null)
        streamRequest = supportsRequestStreams;
    if (timeout != null)
        signal = timeoutSignal(timeout, new Error("Network request timed out"), signal);
    ...
    if (resp.headers.get("content-type") !== "application/x-multiple-json") {
        ...
        return objectMap(schema, (schemaValue, key) => decoder.getParam(key).catch(e => {
            if (isOptionalField(schemaValue))
                return undefined;
            throw e;
        }).then(value => {
