...
export class Intermediary {
    ...
    async getReputation(chainIdentifier, swapContract, tokens, abortSignal) {
        ...
        for (let token of checkReputationTokens) {
            promises.push(tryWithRetries(() => swapContract.getIntermediaryReputation(this.getAddress(chainIdentifier), token), undefined, undefined, abortSignal).then(result => {
                if (result != null)
    ...
    async getLiquidity(chainIdentifier, swapContract, token, abortSignal) {
        const result = await tryWithRetries(() => swapContract.getBalance(this.getAddress(chainIdentifier), token, true), undefined, undefined, abortSignal);
        this.liquidity ??= {};
