...
export class IntermediaryDiscovery extends EventEmitter {
    ...
    async getIntermediaryUrls(abortSignal) {
        ...
        const response = await tryWithRetries(() => httpGet(this.registryUrl, this.httpRequestTimeout, abortSignal), { maxRetries: 3, delay: 100, exponential: true });
    ...
    async getNodeInfo(url, abortSignal) {
        const response = await tryWithRetries(() => this.lpApi.getIntermediaryInfo(url, this.httpRequestTimeout, abortSignal), { maxRetries: 3, delay: 100, exponential: true }, undefined, abortSignal, "debug");
    ...
    getSwapCandidates(chainIdentifier, swapType, tokenAddress, amount, count) {
        const candidates = this.intermediaries.filter(e => {
            ...
            if (contracts == null)
                return false;
            if (swapType === SwapType.FROM_BTCLN_AUTO && !contracts.swapContract?.supportsInitWithoutClaimer)
                return false;
            if (swapType === SwapType.SPV_VAULT_FROM_BTC && contracts.spvVaultContract == null)
                return false;
            return true;
        });
