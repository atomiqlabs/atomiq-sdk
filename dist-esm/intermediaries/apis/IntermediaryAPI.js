...
/////////////////////////
///// Trusted from BTCLN
export var TrustedInvoiceStatusResponseCodes;
...
/////////////////////////
///// Trusted from BTC
export var TrustedAddressStatusResponseCodes;
...
export class IntermediaryAPI {
    ...
    async getRefundAuthorization(url, paymentHash, sequence, timeout, abortSignal) {
        return tryWithRetries(() => this.httpGet(url + "/getRefundAuthorization" +
            "?paymentHash=" + encodeURIComponent(paymentHash) +
            "&sequence=" + encodeURIComponent(sequence.toString(10)), timeout, abortSignal), undefined, RequestError, abortSignal);
    }
    /**
    ...
    async getPaymentAuthorization(url, paymentHash, timeout, abortSignal) {
        return tryWithRetries(() => this.httpGet(url + "/getInvoicePaymentAuth" +
            "?paymentHash=" + encodeURIComponent(paymentHash), timeout, abortSignal), undefined, RequestError, abortSignal);
    }
    /**
    ...
    async getInvoiceStatus(url, paymentHash, timeout, abortSignal) {
        return tryWithRetries(() => this.httpGet(url + "/getInvoiceStatus" +
            "?paymentHash=" + encodeURIComponent(paymentHash), timeout, abortSignal), undefined, RequestError, abortSignal);
    }
    /**
    ...
    initToBTC(chainIdentifier, baseUrl, init, timeout, abortSignal, streamRequest) {
        ...
        return {
            ...
            response: responseBodyPromise.then((responseBody) => Promise.all([
                ...
                if (result == null)
                    throw new RequestError("Cannot parse the response with the expected schema", 200);
                return result;
            })
        };
    }
    ...
    async getTrustedInvoiceStatus(url, paymentHash, timeout, abortSignal) {
        return tryWithRetries(() => this.httpGet(url + "/getInvoiceStatus?paymentHash=" + encodeURIComponent(paymentHash), timeout, abortSignal), undefined, RequestError, abortSignal);
    }
    /**
    ...
    async initTrustedFromBTCLN(chainIdentifier, baseUrl, init, timeout, abortSignal) {
        const resp = await tryWithRetries(() => this.httpGet(baseUrl + "/lnforgas/createInvoice" +
            ...
            "&token=" + encodeURIComponent(init.token), timeout, abortSignal), undefined, RequestError, abortSignal);
        if (resp.code !== 10000)
            throw RequestError.parse(JSON.stringify(resp), 400);
    ...
    async getTrustedAddressStatus(url, paymentHash, sequence, timeout, abortSignal) {
        return tryWithRetries(() => this.httpGet(url + "/getAddressStatus?paymentHash=" + encodeURIComponent(paymentHash) + "&sequence=" + encodeURIComponent(sequence.toString(10)), timeout, abortSignal), undefined, RequestError, abortSignal);
    }
    /**
    ...
    async setTrustedRefundAddress(url, paymentHash, sequence, refundAddress, timeout, abortSignal) {
        return tryWithRetries(() => this.httpGet(url + "/setRefundAddress" +
            "?paymentHash=" + encodeURIComponent(paymentHash) +
            "&sequence=" + encodeURIComponent(sequence.toString(10)) +
            "&refundAddress=" + encodeURIComponent(refundAddress), timeout, abortSignal), undefined, RequestError, abortSignal);
    }
    /**
    ...
    async initTrustedFromBTC(chainIdentifier, baseUrl, init, timeout, abortSignal) {
        const resp = await tryWithRetries(() => this.httpGet(baseUrl + "/frombtc_trusted/getAddress?chain=" + encodeURIComponent(chainIdentifier) +
            "&address=" + encodeURIComponent(init.address) +
            "&amount=" + encodeURIComponent(init.amount.toString(10)) +
