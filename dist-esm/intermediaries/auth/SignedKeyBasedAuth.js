...
function parsePublicKey(value, name) {
    ...
    if (!secp256k1.utils.isValidPublicKey(Buffer.from("02" + value, "hex"), true))
 ...
 */
export function getSignedKeyBasedAuthHandler(certificate, privateKey) {
    ...
    if (!schnorr.verify(authoritySignature, signingPublicKey, authorityPublicKey))
        throw new Error("Invalid certificate, authority signature is not valid!");
    if (!signingPublicKey.equals(schnorr.getPublicKey(privateKey)))
