...
export class SwapperUtils {
    ...
    createBitcoinWalletFromMnemonic(mnemonic, options) {
        return SingleAddressBitcoinWallet.fromMnemonic(this.root._bitcoinRpc, this.bitcoinNetwork, mnemonic, options?.derivationPath, options?.feeMultiplier, options?.feeOverride);
    }
    /**
    ...
    createBitcoinWalletFromEntropy(entropy, options) {
        return this.createBitcoinWalletFromMnemonic(SingleAddressBitcoinWallet.mnemonicFromEntropy(Buffer.from(entropy)), options);
    }
    /**
    ...
    async getSpendableBalance(wallet, token, options) {
        ...
        if (chainInterface.getNativeCurrencyAddress() !== token.address) {
            ...
            let [balance, commitFee] = await Promise.all([
                ...
                await swapContract.createSwapData(ChainSwapType.HTLC, signer, chainInterface.randomAddress(), token.address, 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffn, swapContract.getHashForHtlc(randomBytes(32)).toString("hex"), BigIntBufferUtils.fromBuffer(randomBytes(8)), BigInt(Math.floor(Date.now() / 1000)), true, false, BigIntBufferUtils.fromBuffer(randomBytes(2)), BigIntBufferUtils.fromBuffer(randomBytes(2))), options?.feeRate)
            ]);
