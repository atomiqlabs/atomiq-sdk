...
})(FromBTCLNSwapState || (FromBTCLNSwapState = {}));
...
export function isFromBTCLNSwapInit(obj) {
 ...
 * @category Swaps/Legacy/Lightning → Smart chain
 */
export class FromBTCLNSwap extends IFromBTCSelfInitSwap {
    /**
            ...
            if (this.canCommitAndClaimInOneShot()) {
                ...
                if (this._state === FromBTCLNSwapState.PR_PAID) {
                    ...
                    if (options?.delayBetweenCommitAndClaimSeconds != null)
                        await timeoutPromise(options.delayBetweenCommitAndClaimSeconds * 1000, options?.abortSignal);
                }
    ...
    async waitForPayment(onPaymentReceived, checkIntervalSeconds, abortSignal) {
        ...
        let resp = { code: PaymentAuthorizationResponseCodes.PENDING, msg: "" };
    ...
    async getCommitAndClaimNetworkFee() {
        ...
        return toTokenAmount(commitFee + claimFee, this.wrapper._getNativeToken(), this.wrapper._prices);
    }
    /**
