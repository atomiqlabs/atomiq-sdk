...
export function isFromBTCLNAutoSwapInit(obj) {
 ...
 */
export class FromBTCLNAutoSwap extends IEscrowSwap {
    /**
    ...
    constructor(wrapper, initOrObject) {
         ...
         * @internal
         */
        this.inputToken = BitcoinTokens.BTCLN;
     ...
     */
    getGasDropOutput() {
        return toTokenAmount(this.getSwapData().getSecurityDeposit() - this.getSwapData().getClaimerBounty(), this.wrapper._tokens[this.getSwapData().getDepositToken()], this.wrapper._prices, this.gasPricingInfo);
    }
    /**
    ...
    getFee() {
        ...
        const amountInSrcToken = toTokenAmount(swapFee.amountInSrcToken.rawAmount + watchtowerFee.amountInSrcToken.rawAmount, BitcoinTokens.BTCLN, this.wrapper._prices, this.pricingInfo);
        return {
            amountInSrcToken,
            amountInDstToken: toTokenAmount(swapFee.amountInDstToken.rawAmount + watchtowerFee.amountInDstToken.rawAmount, this.wrapper._tokens[this.getSwapData().getToken()], this.wrapper._prices, this.pricingInfo),
            currentUsdValue: amountInSrcToken.currentUsdValue,
            usdValue: amountInSrcToken.usdValue,
            pastUsdValue: amountInSrcToken.pastUsdValue
        };
    ...
    async _buildLightningPaymentAction() {
        return {
            ...
            waitForTransactions: async (maxWaitTimeSeconds, pollIntervalSeconds, abortSignal) => {
                const abortController = extendAbortController(abortSignal, maxWaitTimeSeconds, "Timed out waiting for lightning payment");
    ...
    async _buildWaitLpAction() {
        return {
            ...
            wait: async (maxWaitTimeSeconds, pollIntervalSeconds, abortSignal) => {
                const abortController = extendAbortController(abortSignal, maxWaitTimeSeconds, "Timed out waiting for LP payout");
    ...
    async _waitForLpPaymentReceived(checkIntervalSeconds, abortSignal) {
        ...
        if (this._state === FromBTCLNAutoSwapState.PR_CREATED) {
            ...
            if (this.url != null)
                promises.push((async () => {
                    let resp = { code: InvoiceStatusResponseCodes.PENDING, msg: "" };
    ...
    async checkIntermediaryReturnedData(data) {
        if (!data.isPayOut())
            throw new IntermediaryError("Invalid not pay out");
        if (data.getType() !== ChainSwapType.HTLC)
            throw new IntermediaryError("Invalid swap type");
        ...
        if (!data.isClaimer(this._getInitiator()))
            throw new IntermediaryError("Invalid claimer used");
        ...
        if (data.getClaimHash() !== this.getSwapData().getClaimHash())
            throw new IntermediaryError("Invalid payment hash used!");
        if (!data.isDepositToken(this.getSwapData().getDepositToken()))
            throw new IntermediaryError("Invalid deposit token used!");
        if (data.hasSuccessAction())
            throw new IntermediaryError("Invalid has success action");
        if (await this.wrapper._contract(this._contractVersion).isExpired(this._getInitiator(), data))
            throw new IntermediaryError("Not enough time to claim!");
        if (this.wrapper._getHtlcTimeout(data) <= (Date.now() / 1000))
            throw new IntermediaryError("HTLC expires too soon!");
    }
    /**
     * Waits till a lightning network payment is received by the intermediary, and the intermediary
     *  initiates the swap HTLC on the smart chain side. After the HTLC is initiated you can wait
