...
import { extendAbortController, mapArrayToObject, parseHashValueExact32Bytes, randomBytes, throwIfUndefined } from "../../../../utils/Utils.js";
import { RequestError } from "../../../../errors/RequestError.js";
import { FromBTCLNAutoSwap, FromBTCLNAutoSwapState } from "./FromBTCLNAutoSwap.js";
 ...
 */
export class FromBTCLNAutoWrapper extends IFromBTCLNWrapper {
    /**
    ...
    constructor(chainIdentifier, unifiedStorage, unifiedChainEvents, chain, prices, tokens, versionedContracts, lnApi, messenger, lpApi, options, events) {
        ...
        this.TYPE = SwapType.FROM_BTCLN_AUTO;
        /**
         * @internal
         */
        this.tickSwapState = [
         ...
         * @internal
         */
        this._claimableSwapStates = [FromBTCLNAutoSwapState.CLAIM_COMMITED];
        /**
         * @internal
         */
        this._swapDeserializer = FromBTCLNAutoSwap;
    ...
    preFetchClaimerBounty(signer, amountData, options, abortController, contractVersions) {
        return mapArrayToObject(contractVersions, async (contractVersion) => {
            ...
            const dummySwapData = await this._contract(contractVersion).createSwapData(ChainSwapType.HTLC, this._chain.randomAddress(), signer, amountData.token, dummyAmount, this._contract(contractVersion).getHashForHtlc(randomBytes(32)).toString("hex"), this.getRandomSequence(), BigInt(Math.floor(Date.now() / 1000)), false, true, BigInt(Math.floor(Math.random() * 0x10000)), BigInt(Math.floor(Math.random() * 0x10000)));
            try {
    ...
    create(recipient, amountData, lps, options, additionalParams, abortSignal, preFetches) {
        ...
        return lps.map(lp => {
            return {
                intermediary: lp,
                quote: (async () => {
                    if (lp.services[SwapType.FROM_BTCLN_AUTO] == null)
                    ...
                    try {
                        ...
                        const [pricingInfo, gasPricingInfo] = await Promise.all([
                            this.verifyReturnedPrice(lp.services[SwapType.FROM_BTCLN_AUTO], false, resp.btcAmountSwap, resp.total, amountData.token, { swapFeeBtc: resp.swapFeeBtc }, _preFetches.pricePrefetchPromise, _preFetches.usdPricePrefetchPromise, abortController.signal),
                            _options.gasAmount === 0n ? Promise.resolve(undefined) : this.verifyReturnedPrice({ ...lp.services[SwapType.FROM_BTCLN_AUTO], swapBaseFee: 0 }, //Base fee should be charged only on the amount, not on gas
                        ...
                        const swapInit = {
                            ...
                            gasPricingInfo,
                            initialSwapData: await this._contract(version).createSwapData(ChainSwapType.HTLC, lp.getAddress(this.chainIdentifier), recipient, amountData.token, resp.total, _hash[version], this.getRandomSequence(), BigInt(Math.floor(Date.now() / 1000)), false, true, _options.gasAmount + resp.claimerBounty, resp.claimerBounty, nativeTokenAddress),
                            pr: resp.pr,
                            secret: secret?.toString("hex"),
                            exactIn: amountData.exactIn ?? true,
                            contractVersion: version
                        };
                        const quote = new FromBTCLNAutoSwap(this, swapInit);
                        return quote;
                    }
                    catch (e) {
                        abortController.abort(e);
