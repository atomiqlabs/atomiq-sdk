...
export function isFromBTCSwapInit(obj) {
 ...
 * @category Swaps/Legacy/Bitcoin → Smart chain
 */
export class FromBTCSwap extends IFromBTCSelfInitSwap {
    constructor(wrapper, initOrObject) {
         ...
         * @internal
         */
        this.inputToken = BitcoinTokens.BTC;
    ...
    inferRequiredConfirmationsCount(btcTx, vout) {
        ...
        for (let i = 1; i <= 20; i++) {
            const computedClaimHash = this._contract.getHashForOnchain(Buffer.from(txOut.scriptPubKey.hex, "hex"), BigInt(txOut.value), i);
    ...
    async waitForBitcoinTransaction(updateCallback, checkIntervalSeconds, abortSignal) {
        ...
        const result = await this.wrapper._btcRpc.waitForAddressTxo(this.address, Buffer.from(txoHashHint, "hex"), this.requiredConfirmations ?? 6, //In case confirmation count is not known, we use a conservative estimate
        (btcTx, vout, txEtaMs) => {
    ...
    async _buildWaitBitcoinConfirmationsAction(confirmationDelay, description) {
        return {
            ...
            wait: async (maxWaitTimeSeconds, pollIntervalSeconds, abortSignal, btcConfirmationsCallback) => {
                const abortController = extendAbortController(abortSignal, maxWaitTimeSeconds, "Timed out waiting for bitcoin transaction to confirm");
