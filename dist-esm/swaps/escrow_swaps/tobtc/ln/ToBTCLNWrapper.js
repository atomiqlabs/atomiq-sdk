 ...
 */
export class ToBTCLNWrapper extends IToBTCWrapper {
    constructor(chainIdentifier, unifiedStorage, unifiedChainEvents, chain, prices, tokens, versionedContracts, lpApi, options, events) {
         ...
         * @internal
         */
        this._swapDeserializer = ToBTCLNSwap;
    }
    ...
    async getIntermediaryQuote(signer, amountData, lp, pr, parsedPr, calculatedOptions, preFetches, abort, additionalParams) {
        if (lp.services[SwapType.TO_BTCLN] == null)
        ...
        try {
            ...
            const [pricingInfo, signatureExpiry, reputation] = await Promise.all([
                this.verifyReturnedPrice(lp.services[SwapType.TO_BTCLN], true, amountOut, data.getAmount(), amountData.token, { networkFee: resp.maxFee, swapFeeBtc }, preFetches.pricePreFetchPromise, preFetches.usdPricePrefetchPromise, abortController.signal),
            ...
            const quote = new ToBTCLNSwap(this, {
                pricingInfo,
                url: lp.url,
    ...
    async create(signer, recipient, amountData, lps, options, additionalParams, abortSignal, preFetches) {
        ...
        const _preFetches = preFetches ?? {
            ...
            signDataPrefetchPromise: mapArrayToObject(lpVersions, (contractVersion) => {
    ...
    async getIntermediaryQuoteExactIn(signer, amountData, invoiceCreateService, lp, dummyPr, calculatedOptions, preFetches, abortSignal, additionalParams) {
        ...
        const reputationPromise = this.preFetchIntermediaryReputation(amountData, lp, abortController, version);
        try {
            const { signDataPromise, prepareResp } = await tryWithRetries(async (retryCount) => {
            ...
            const resp = await tryWithRetries((retryCount) => this._lpApi.initToBTCLNExactIn(lp.url, {
                pr: invoice,
                reqId: prepareResp.reqId,
                feeRate: throwIfUndefined(preFetches.feeRatePromise[version], "Network fee rate pre-fetch failed!"),
                additionalParams
            }, this._options.postRequestTimeout, abortController.signal, retryCount > 0 ? false : undefined), undefined, RequestError, abortController.signal);
            ...
            const swapFeeBtc = resp.swapFee * amountOut / (data.getAmount() - totalFee);
            const [pricingInfo, signatureExpiry, reputation] = await Promise.all([
                this.verifyReturnedPrice(lp.services[SwapType.TO_BTCLN], true, prepareResp.amount, data.getAmount(), amountData.token, { networkFee: resp.maxFee, swapFeeBtc }, preFetches.pricePreFetchPromise, preFetches.usdPricePrefetchPromise, abortSignal),
                this.verifyReturnedSignature(signer, data, resp, preFetches.feeRatePromise[version], signDataPromise, version, abortController.signal),
                reputationPromise
            ]);
            abortController.signal.throwIfAborted();
    ...
    async createViaInvoiceCreateService(signer, invoiceCreateServicePromise, amountData, lps, options, additionalParams, abortSignal) {
        ...
        const signDataPrefetchPromise = mapArrayToObject(lpVersions, (contractVersion) => {
            return this._contract(contractVersion).preFetchBlockDataForSignatures == null ?
                this.preFetchSignData(Promise.resolve(true), contractVersion) :
                undefined;
        });
        ...
        const invoiceCreateService = (async () => {
            ...
            return {
                getInvoice: async (amountSats, abortSignal) => {
                    const { invoice, successAction } = await LNURL.useLNURLPay(payRequest, BigInt(amountSats), options?.comment, this._options.getRequestTimeout, abortSignal);
