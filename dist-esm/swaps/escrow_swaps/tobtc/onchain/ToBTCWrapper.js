 ...
 */
export class ToBTCWrapper extends IToBTCWrapper {
    /**
    ...
    constructor(chainIdentifier, unifiedStorage, unifiedChainEvents, chain, prices, tokens, versionedContracts, btcRpc, lpApi, options, events) {
         ...
         * @internal
         */
        this._swapDeserializer = ToBTCSwap;
        this._btcRpc = btcRpc;
    }
    /**
    ...
    create(signer, recipient, amountData, lps, options, additionalParams, abortSignal) {
        ...
        const _hash = !amountData.exactIn ?
            mapArrayToObject(lpVersions, (contractVersion) => {
        ...
        return lps.map(lp => {
            return {
                intermediary: lp,
                quote: (async () => {
                    if (lp.services[SwapType.TO_BTC] == null)
                    ...
                    try {
                        ...
                        const [pricingInfo, signatureExpiry, reputation] = await Promise.all([
                            this.verifyReturnedPrice(lp.services[SwapType.TO_BTC], true, resp.amount, data.getAmount(), amountData.token, { networkFee: resp.networkFee, swapFeeBtc }, pricePreFetchPromise, usdPricePrefetchPromise, abortController.signal),
                        ...
                        const quote = new ToBTCSwap(this, {
                            pricingInfo,
                            url: lp.url,
