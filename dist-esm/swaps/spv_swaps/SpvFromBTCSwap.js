import { SpvFromBTCSwapState } from "./SpvFromBTCSwapState.js";
...
export function isSpvFromBTCSwapInit(obj) {
 ...
 */
export class SpvFromBTCSwap extends ISwap {
     ...
     */
    getOutputWithoutFee() {
        return toTokenAmount((this.outputTotalSwap * (100000n + this.callerFeeShare + this.frontingFeeShare + this.executionFeeShare) / 100000n) + (this.swapFee ?? 0n), this.wrapper._tokens[this.outputSwapToken], this.wrapper._prices, this.pricingInfo);
    }
    /**
    ...
    getSwapFee() {
        ...
        const amountInSrcToken = toTokenAmount(this.swapFeeBtc + this.gasSwapFeeBtc, BitcoinTokens.BTC, this.wrapper._prices, this.pricingInfo);
        return {
            amountInSrcToken,
    ...
    getWatchtowerFee() {
        ...
        return {
            amountInSrcToken,
            amountInDstToken: toTokenAmount((this.outputTotalSwap * (totalFeeShare + this.executionFeeShare) / 100000n) + watchtowerFeeInOutputToken, outputToken, this.wrapper._prices, this.pricingInfo),
            currentUsdValue: amountInSrcToken.currentUsdValue,
            usdValue: amountInSrcToken.usdValue,
            pastUsdValue: amountInSrcToken.pastUsdValue
        };
    ...
    getFee() {
        ...
        const amountInSrcToken = toTokenAmount(swapFee.amountInSrcToken.rawAmount + watchtowerFee.amountInSrcToken.rawAmount, BitcoinTokens.BTC, this.wrapper._prices, this.pricingInfo);
        return {
            amountInSrcToken,
            amountInDstToken: toTokenAmount(swapFee.amountInDstToken.rawAmount + watchtowerFee.amountInDstToken.rawAmount, this.wrapper._tokens[this.outputSwapToken], this.wrapper._prices, this.pricingInfo),
            currentUsdValue: amountInSrcToken.currentUsdValue,
            usdValue: amountInSrcToken.usdValue,
            pastUsdValue: amountInSrcToken.pastUsdValue
        };
        ...
        if (additionalOutputs != null)
            additionalOutputs.forEach(output => {
                psbt.addOutput({
                    ...
                    script: output.outputScript ?? toOutputScript(this.wrapper._options.bitcoinNetwork, output.address)
                });
            });
    ...
    async _buildWaitBitcoinConfirmationsAction(confirmationDelay) {
        return {
            ...
            wait: async (maxWaitTimeSeconds, pollIntervalSeconds, abortSignal, btcConfirmationsCallback) => {
                const abortController = extendAbortController(abortSignal, maxWaitTimeSeconds, "Timed out waiting for bitcoin transaction");
    ...
    async watchdogWaitTillResult(interval = 5, abortSignal) {
        ...
        let status = { type: SpvWithdrawalStateType.NOT_FOUND };
        while (status.type === SpvWithdrawalStateType.NOT_FOUND) {
            ...
            try {
                ...
                if (await this._shouldCheckWithdrawalState()) {
                    status = await this._contract.getWithdrawalState(this._data, this._genesisSmartChainBlockHeight) ?? { type: SpvWithdrawalStateType.NOT_FOUND };
                }
            }
            catch (e) {
