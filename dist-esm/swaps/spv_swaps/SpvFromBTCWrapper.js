...
import { bigIntCeilDivision, extendAbortController, mapArrayToObject, randomBytes, throwIfUndefined } from "../../utils/Utils.js";
import { fromOutputScript, getDummyOutputScript, toCoinselectAddressType, toOutputScript } from "../../utils/BitcoinUtils.js";
...
export const REQUIRED_SPV_SWAP_VAULT_ADDRESS_TYPE = "p2tr";
export const REQUIRED_SPV_SWAP_LP_ADDRESS_TYPE = "p2wpkh";
export const DEFAULT_CPFP_ASSUMPTION = { txVsize: 200, txEffectiveFeeRate: 1 };
export function assertSupportedSpvFundingType(type) {
    if (type !== "p2wpkh" && type !== "p2sh-p2wpkh" && type !== "p2tr") {
        throw new UserError(`Unsupported SPV funding address type: ${type}. Supported types: p2wpkh, p2sh-p2wpkh, p2tr`);
    }
}
/**
 ...
 */
export class SpvFromBTCWrapper extends ISwapWrapper {
    /**
    ...
    constructor(chainIdentifier, unifiedStorage, unifiedChainEvents, chain, prices, tokens, versionedContracts, versionedSynchronizer, btcRpc, lpApi, options, events) {
        ...
        this.TYPE = SwapType.SPV_VAULT_FROM_BTC;
        /**
         * @internal
         */
        this._claimableSwapStates = [SpvFromBTCSwapState.BTC_TX_CONFIRMED];
        /**
         * @internal
         */
        this._swapDeserializer = SpvFromBTCSwap;
        /**
         * @internal
         */
    ...
    async processEventFront(event, swap) {
        if (swap._state === SpvFromBTCSwapState.SIGNED || swap._state === SpvFromBTCSwapState.POSTED ||
    ...
    async computeCallerFee(amountPrefetch, totalFeeInNativeTokenPrefetch, amountData, options, pricePrefetch, nativeTokenPricePrefetch, abortSignal) {
        ...
        let callerFeeShare = payoutAmount > 0
            ? bigIntCeilDivision(totalFeeInNativeToken * 100000n, payoutAmount)
    ...
    async verifyReturnedData(resp, amountData, lp, options, callerFee, maxBitcoinFeeRatePromise, bitcoinFeeRatePromise, abortSignal) {
        ...
        if (btcFeeRate != null && resp.btcFeeRate > btcFeeRate)
            throw new IntermediaryError(`Required bitcoin fee rate returned from the LP is too high! Maximum accepted: ${btcFeeRate} sats/vB, required by LP: ${resp.btcFeeRate} sats/vB`);
        ...
        if (resp.usedExactFeeCalculation) {
            if (amountData.exactIn) {
                ...
                const expectedCallerFeeShare = bigIntCeilDivision(callerFee.callerFee * 100000n, resp.btcAmount - callerFee.callerFee);
                if (resp.callerFeeShare !== expectedCallerFeeShare)
                    throw new IntermediaryError("Invalid caller fee returned (EXACT_IN mode)");
            }
            else {
                const expectedCallerFeeShare = bigIntCeilDivision(callerFee.callerFee * 100000n, amountData.amount);
                if (resp.callerFeeShare !== expectedCallerFeeShare)
                    throw new IntermediaryError("Invalid caller fee returned (EXACT_OUT mode)");
            }
        }
        else {
            if (resp.callerFeeShare !== callerFee.callerFeeShare)
                throw new IntermediaryError("Invalid caller fee returned");
        }
        //Check expiry
        const timeNowSeconds = Math.floor(Date.now() / 1000);
        if (resp.expiry < timeNowSeconds)
            throw new IntermediaryError(`Quote already expired, expiry: ${resp.expiry}, systemTime: ${timeNowSeconds}, clockAdjusted: ${Date._now != null}`);
        ...
        let [{ vault, utxoSelection }, { vaultUtxoValue, btcTx }] = await Promise.all([
            (async () => {
                ...
                if (vault == null || !vault.isOpened())
                    throw new IntermediaryError("Returned spv swap vault is not opened!");
                ...
                if (vault.getConfirmations() > this._options.maxConfirmations)
                    throw new IntermediaryError("SPV swap vault needs too many confirmations: " + vault.getConfirmations());
                ...
                if (amountData.exactIn) {
                    if (options.sourceWalletUtxos == null) {
                        ...
                        if (!resp.usedUtxoInputCalculation)
                            throw new IntermediaryError("Invalid usedUtxoInputCalculation flag returned");
                        ...
                        if (resp.btcAmount !== lpOutputAmount)
                            throw new IntermediaryError(`Invalid amount returned, expected: ${lpOutputAmount.toString(10)}, got: ${resp.btcAmount.toString(10)}`);
                    }
                }
                else {
            ...
            (async () => {
                ...
                if (btcTx == null)
                    throw new IntermediaryError("Invalid UTXO, doesn't exist (txId)");
                abortController.signal.throwIfAborted();
                if (btcTx.confirmations == null || btcTx.confirmations < 1)
                    throw new IntermediaryError("SPV vault UTXO not confirmed");
                const vout = parseInt(voutStr);
                if (btcTx.outs[vout] == null)
                    throw new IntermediaryError("Invalid UTXO, doesn't exist");
            ...
            (async () => {
                ...
                if (await this._btcRpc.isSpent(utxo))
                    throw new IntermediaryError("Returned spv vault UTXO is already spent", null, true);
                abortController.signal.throwIfAborted();
            })()
        ]).catch(e => {
            abortController.abort(e);
        ...
        while (vault.getUtxo() !== utxo) {
            ...
            if (btcTx.txid !== txId) {
                ...
                if (_btcTx == null)
                    throw new IntermediaryError("Invalid ancestor transaction (not found)");
                btcTx = _btcTx;
            }
    ...
    async calculateSelectedUtxosAndAmounts(walletUtxos, bitcoinFeeRate, includeGas, amount, walletType, skipDetrimental = true, cpfpAssumption) {
        ...
        let spendableBalance = await BitcoinWallet.getSpendableBalance(walletUtxos, bitcoinFeeRate, swapPsbt, REQUIRED_SPV_SWAP_LP_ADDRESS_TYPE, skipDetrimental);
        ...
        if (amount != null) {
            ...
            if (difference < 0n) {
                ...
                spendableBalance = BitcoinWallet.getSpendableBalance([...walletUtxos, syntheticUtxo], bitcoinFeeRate, swapPsbt, REQUIRED_SPV_SWAP_LP_ADDRESS_TYPE, skipDetrimental);
                ...
                const swapPsbt = this.getDummySwapPsbt(includeGas);
                spendableBalance = BitcoinWallet.getSpendableBalance(walletUtxos, bitcoinFeeRate, swapPsbt, REQUIRED_SPV_SWAP_LP_ADDRESS_TYPE, skipDetrimental);
            }
            else {
    ...
    async amountPrefetch(amountData, bitcoinFeeRatePromise, walletUtxosPromise, includeGas, abortController, walletType, skipDetrimental, sourceWalletCpfpAssumption) {
        try {
            if (walletUtxosPromise == null) {
                if (amountData.amount == null)
                    throw new UserError("Amount has to be specified when not passing UTXOs!");
                return amountData.amount;
            }
            else {
                if (amountData.amount != null) {
                    if (walletType == null)
                        throw new UserError("Wallet type has to be specified when passing UTXOs and amount!");
                }
    ...
    _create(recipient, amountData, lps, options, additionalParams, abortSignal) {
        ...
        const callerFeePrefetchPromise = mapArrayToObject(lpVersions, (contractVersion) => {
        ...
        return lps.map(lp => {
            return {
                ...
                result: tryWithRetries(async () => {
                    if (lp.services[SwapType.SPV_VAULT_FROM_BTC] == null)
                    ...
                    try {
                        const resp = await tryWithRetries(async (retryCount) => {
                            return await this._lpApi.prepareSpvFromBTC(this.chainIdentifier, lp.url, {
                                ...
                                amountChangeVSize: initialSelectedUtxosInfo?.then(value => value?.changeOutputAmount != null ? utils.outputBytes({ type: _options.sourceWalletAddressType }) : undefined),
                                additionalParams
                        ...
                        const [pricingInfo, gasPricingInfo, { vault, vaultUtxoValue, utxoSelection }] = await Promise.all([
                            this.verifyReturnedPrice(lp.services[SwapType.SPV_VAULT_FROM_BTC], false, resp.btcAmountSwap, resp.total * (100000n + resp.callerFeeShare) / 100000n, amountData.token, { swapFeeBtc: resp.swapFeeBtc }, pricePrefetchPromise, usdPricePrefetchPromise, abortController.signal),
                            _options.gasAmount === 0n ? Promise.resolve(undefined) : this.verifyReturnedPrice({ ...lp.services[SwapType.SPV_VAULT_FROM_BTC], swapBaseFee: 0 }, //Base fee should be charged only on the amount, not on gas
                        ...
                        if (bitcoinFeeRatePromise != null)
                            minimumBtcFeeRate = Math.max(minimumBtcFeeRate, await throwIfUndefined(bitcoinFeeRatePromise));
                        const swapInit = {
                            pricingInfo,
                            url: lp.url,
                            ...
                            genesisSmartChainBlockHeight: await throwIfUndefined(finalizedBlockHeightPrefetchPromise, "Network finalized blockheight pre-fetch failed!"),
                            contractVersion: version
                        };
                        const quote = new SpvFromBTCSwap(this, swapInit);
    ...
    async _checkPastSwaps(pastSwaps) {
        ...
        for (let pastSwap of pastSwaps) {
            let changed = false;
            if (pastSwap._state === SpvFromBTCSwapState.SIGNED ||
