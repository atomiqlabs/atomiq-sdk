...
export function isLnForGasSwapInit(obj) {
 ...
 * @category Swaps/Trusted Gas Swaps
 */
export class LnForGasSwap extends ISwap {
     ...
     */
    getOutput() {
        return toTokenAmount(this.outputAmount, this.wrapper._tokens[this.wrapper._chain.getNativeCurrencyAddress()], this.wrapper._prices, this.pricingInfo);
    }
    /**
     * @inheritDoc
     */
    ...
    getInputWithoutFee() {
        ...
        if (msats == null)
            throw new Error("Swap lightning invoice has no msat amount field!");
        const amount = (BigInt(msats) + 999n) / 1000n;
        return toTokenAmount(amount - (this.swapFeeBtc ?? 0n), BitcoinTokens.BTCLN, this.wrapper._prices, this.pricingInfo);
    }
    /**
    ...
    async waitForPayment(checkIntervalSeconds, abortSignal, onPaymentReceived) {
        ...
        while (!abortSignal?.aborted && (this._state === LnForGasSwapState.PR_CREATED || this._state === LnForGasSwapState.PR_PAID)) {
            ...
            if (this._state === LnForGasSwapState.PR_CREATED || this._state === LnForGasSwapState.PR_PAID)
                await timeoutPromise((checkIntervalSeconds ?? 5) * 1000, abortSignal);
        }
        if (abortSignal != null)
