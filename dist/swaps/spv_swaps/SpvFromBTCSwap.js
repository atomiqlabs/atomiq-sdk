...
exports.SpvFromBTCSwap = void 0;
const SpvFromBTCWrapper_js_1 = require("./SpvFromBTCWrapper.js");
...
class SpvFromBTCSwap extends SpvFromBTCSwapBase_1.SpvFromBTCSwapBase {
            ...
            if (info != null)
                this.externalSwapModeInfo = {
                    ...
                    selectedExistingUtxos: info.selectedExistingUtxos.map((utxo) => ({
                        ...utxo,
                        outputScript: buffer_1.Buffer.from(utxo.outputScript, "hex"),
                    })),
    ...
    getNetworkInputFee() {
        ...
        const amountInSrcToken = (0, TokenAmount_js_1.toTokenAmount)(networkFee, Token_js_1.BitcoinTokens.BTC, this.wrapper._prices, this.pricingInfo);
        return {
            amountInSrcToken,
            amountInDstToken: (0, TokenAmount_js_1.toTokenAmount)(networkFeeInOutputToken, outputToken, this.wrapper._prices, this.pricingInfo),
    ...
    getFinalizedCoinselect(inputs, outputs, feeRate = this.minimumBtcFeeRate, changeType = null) {
        ...
        return utils_1.utils.finalize([
            ...
            {
                value: Number(txDetails.vaultAmount),
                script: buffer_1.Buffer.from(txDetails.vaultScript)
            },
            {
                value: 0,
                script: buffer_1.Buffer.from(txDetails.out1script)
            },
            {
                value: Number(txDetails.out2amount),
                script: buffer_1.Buffer.from(txDetails.out2script)
            },
            ...outputs
        ], feeRate, changeType);
    }
    ...
    async setSwapModeIntermediateWallet(intermediateWallet, existingUtxos, feeRate, cpfpAssumptions) {
        ...
        existingUtxos.forEach(utxo => (0, SpvFromBTCWrapper_js_1.assertSupportedSpvFundingType)(utxo.type));
        ...
        const resolvedCpfpAssumptions = cpfpAssumptions ?? SpvFromBTCWrapper_js_1.DEFAULT_CPFP_ASSUMPTION;
        let totalNetworkFee;
    ...
    getExternalDepositAmount() {
        ...
        if (info.requiredDeposit == null)
            return null;
        return (0, TokenAmount_js_1.toTokenAmount)(info.requiredDeposit.amount, Token_js_1.BitcoinTokens.BTC, this.wrapper._prices, this.pricingInfo);
    }
    /**
    ...
    getInput() {
        ...
        return (0, TokenAmount_js_1.toTokenAmount)(super.getInput().rawAmount + this.externalSwapModeInfo.totalNetworkFee, Token_js_1.BitcoinTokens.BTC, this.wrapper._prices, this.pricingInfo);
    }
    /**
     * @inheritDoc
     */
    getFee() {
        ...
        const amountInSrcToken = (0, TokenAmount_js_1.toTokenAmount)(swapFee.amountInSrcToken.rawAmount + watchtowerFee.amountInSrcToken.rawAmount + (networkInputFee?.amountInSrcToken.rawAmount ?? 0n), Token_js_1.BitcoinTokens.BTC, this.wrapper._prices, this.pricingInfo);
        return {
            amountInSrcToken,
            amountInDstToken: (0, TokenAmount_js_1.toTokenAmount)(swapFee.amountInDstToken.rawAmount + watchtowerFee.amountInDstToken.rawAmount + (networkInputFee?.amountInDstToken.rawAmount ?? 0n), this.getOutputToken(), this.wrapper._prices, this.pricingInfo),
            currentUsdValue: amountInSrcToken.currentUsdValue,
            usdValue: amountInSrcToken.usdValue,
            pastUsdValue: amountInSrcToken.pastUsdValue
        };
    ...
    getFeeBreakdown() {
        ...
        return [
            {
                type: FeeType_js_1.FeeType.NETWORK_INPUT,
                fee: networkInputFee
            },
            ...baseBreakdown
        ];
    ...
    getMatchingExternalDepositUtxo(rehydratedWalletUtxos, ignoredUtxoKeys) {
        ...
        const rehydratedWalletUtxosMap = (0, BitcoinUtils_js_1.toUtxoMap)(rehydratedWalletUtxos);
        const rehydratedSelectedUtxos = info.selectedExistingUtxos.map(utxo => rehydratedWalletUtxosMap.get((0, BitcoinUtils_js_1.getUtxoKey)(utxo)) ?? utxo);
        const selectedUtxosKeys = (0, BitcoinUtils_js_1.toUtxoSet)(info.selectedExistingUtxos);
        const invalidUtxos = [];
        for (const utxo of rehydratedWalletUtxos) {
            ...
            if (utxo.address !== requiredDepositInfo.address)
                continue;
            const key = (0, BitcoinUtils_js_1.getUtxoKey)(utxo);
    ...
    async getIntermediateWalletUtxos(_intermediateWallet) {
        ...
        for (const utxo of info.selectedExistingUtxos) {
            const address = (0, BitcoinUtils_js_1.fromOutputScript)(network, utxo.outputScript.toString("hex"));
        ...
        return (await Promise.all(Array.from(addresses, ([address, addressInfo]) => (0, BitcoinUtils_js_1.getWalletAddressUtxos)(this.wrapper._btcRpc, network, address, addressInfo.publicKey, addressInfo.addressType)))).flat();
    }
    async getFundedPsbt(_bitcoinWallet, feeRate, additionalOutputs, utxos, spendFully) {
        ...
        if (this.externalSwapModeInfo?.changeAmount != null) {
            ...
            psbt.addOutput({
                ...
                script: (0, BitcoinUtils_js_1.toOutputScript)(this.wrapper._options.bitcoinNetwork, changeAddress)
            });
        }
    ...
    async _waitForExternalDeposit(_intermediateWallet, maxWaitTimeSeconds, pollIntervalSeconds, onInvalidDeposit, abortSignal) {
        ...
        try {
            while (this._state !== SpvFromBTCSwapBase_1.SpvFromBTCSwapState.QUOTE_EXPIRED && await this._verifyQuoteValid()) {
                ...
                if (sleepTime <= 0)
                    break;
                await (0, TimeoutUtils_js_1.timeoutPromise)(sleepTime, abortController.signal);
            }
    ...
    async estimateBitcoinFee(_bitcoinWallet, feeRate) {
        ...
        return (0, TokenAmount_js_1.toTokenAmount)(info.totalNetworkFee, Token_js_1.BitcoinTokens.BTC, this.wrapper._prices, this.pricingInfo);
    }
    /**
    ...
    async _buildExternalDepositAddressAction(bitcoinWallet, options) {
        ...
        return {
            ...
            txs: [{
                    ...
                    amount: (0, TokenAmount_js_1.toTokenAmount)(info.requiredDeposit.amount, Token_js_1.BitcoinTokens.BTC, this.wrapper._prices, this.pricingInfo)
                }],
