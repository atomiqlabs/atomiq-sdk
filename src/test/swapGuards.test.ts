...
import {
    ...
    isFromBTCLNAutoSwap,
    isIToBTCSwap,
    isIFromBTCSelfInitSwap,
    isIEscrowSelfInitSwap
} from "../index.js";
 ...
 */
function mockSwap(type: SwapType): ISwap {
    return {getType: () => type} as unknown as ISwap;
}

const ALL_TYPES: SwapType[] = [
    SwapType.FROM_BTC,
    SwapType.FROM_BTCLN,
    SwapType.TO_BTC,
    SwapType.TO_BTCLN,
...
const cases: {name: string, guard: (swap: ISwap) => boolean, truthy: SwapType[]}[] = [
    ...
    test(name + " matches exactly its mapped SwapType(s) across all 8 values", () => {
        for(const type of ALL_TYPES) {
            ...
            strictEqual(
                guard(mockSwap(type)),
                expected,
                `${name}(${SwapType[type]}) should be ${expected}`
            );
        }
    });
}
